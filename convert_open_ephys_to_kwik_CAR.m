function info = convert_open_ephys_to_kwik_CAR(varargin)
...
for processor = 1:size(info.processors,1)
    ...
    if length(recorded_channels) > 0
        ...
        filename_in = [input_directory filesep ...
            ...
            '_CH' int2str(recorded_channels(1)) '.continuous'];
        [data, ~, ~] = load_open_ephys_data(filename_in);
        car = zeros(size(data));
        ...
        car = car/length(recorded_channels);
